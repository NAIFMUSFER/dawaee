...
متابعة 17 سبتمبر: أُضيف محاكي Android متصل بمعاينة مستقلة، بعد عائق الواجهة المحلية الأول. [تفاصيل التشغيل والصور](ANDROID-INTERACTION.md) تثبت النسخ والحدود؛ لم يُتجاوز منع المتصفح المحلي.

## الأجهزة والواجهة

| البيئة | الحجم / اللغة / الخط / الاتصال | المطلوب | النتيجة |
| --- | --- | --- | --- |
| محاكي Android35 صغير | 360×640dp، عربي RTL، خط100%، متصل بمعاينة API0de543b | الدخول، كمية6، أربعة مواعيد،7أيام،23:59، Back ثلاث مرات وإلغاء، حفظ ومخزون60 | PASS على159d62e، تشغيل35204337212؛320×568/خط150%/دون اتصال/الإيماءة/الهاتف الفعلي NOT VERIFIED |
| محاكي Android35 كبير | نحو411×914dp، عربي RTL، خط200%، متصل | نفس رحلة الإدخال والتمرير والحفظ وقراءة القيم من API | PASS على159d62e؛ الإنجليزية،10تكرارات،تعديل/حذف المواعيد باللمس،TalkBack NOT VERIFIED؛ منطق التعديل والحذف ومنع التكرار مختبر آليًا |
...
| متصفح المعاينة المنشورة | معاينة Render العامة، العربية، اتصال فعلي | الاستيقاظ ثم اللغة والدخول وإنشاء الحساب والانتقال لاستعادة كلمة المرور | PASS عرض وتنقل على0de543b؛ لم تُدخل بيانات اعتماد من المتصفح، ولم يُختبر Safari على iPhone؛ SMS BLOCKED |
...
| إضافة الدواء/تعديل/إيقاف/أرشفة | `quick-create-final-audit.test.ts`, `medications.test.ts`, `medication-edit-metadata-roundtrip.test.ts`, `schedule-stop-resume-integrity.test.ts` | PASS آليًا؛ إضافة يدوية باللمس على محاكي بحجمين PASS على159d62e. بقية الرحلات على الجهاز NOT VERIFIED؛ لا دواء يُنشأ في OCR قبل مراجعة المستخدم |
...
| اليوم/التأكيد/السجل/المخزون | `final-audit-dose-stock.test.ts`, `today-final-audit.test.ts`, `stock-ledger-repeat-cycle.test.ts`, `dose-stale-replay-after-undo.test.ts`, `dose-early-action-integrity.test.ts`, `dose-snooze-state-integrity.test.ts`, `dose_check.py` | PASS آليًا؛ f8513aa: تأكيد/تراجع/تأكيد باللمس مع المخزون وبدونه ثم قتل العملية وإعادة الفتح والسجل PASS على محاكي. رصيد60→54→60→54 وحركات-6,+6,-6. الهاتف الفعلي والإنتاج NOT VERIFIED |
...
- صور «بعد» وتسجيل لمس/تمرير: PASS على محاكي Android35/159d62e. الصور أدناه أصلية من screencap، والفيديو الكامل ضمن artifact10489947547 في تشغيل35204337212. هذه بيانات اصطناعية وليست صور هاتف فعلي.
- [نافذة الوقت بخط200%](images/159d62e-large-ar-font200-picker-open.png)، [الكمية6 والوحدة](images/159d62e-large-ar-font200-quantity.png)، [القيمة والمواعيد والمخزون بعد الحفظ](images/159d62e-large-ar-font200-saved.png). أثبت السكربت اللمس والتمرير والرجوع والحفظ بالقراءة من API؛ الصورة منفردة لا تثبت التفاعل.
