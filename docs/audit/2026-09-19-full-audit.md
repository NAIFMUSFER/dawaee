...
| Identity and privacy | Worker email verification checks the exact active relationship. The wrong-password attempt starting a lock remains generic, but correct-password locked responses are still distinct (open finding in the reconciliation). Warm native invitations parse allowed links. Sensitive modals and report output respect account/profile/app-lock boundaries. Emergency QR retry retains its in-memory capability and states offline uncertainty. |
