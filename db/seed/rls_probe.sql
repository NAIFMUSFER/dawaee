...
-- These two positive-control fixtures have completed phone verification.
-- Keep the permission checks meaningful instead of denying all caregiver
-- reads merely because the ownership-proof fixture was missing.
INSERT INTO user_phone_verifications (user_id, phone_e164, authenticated_at)
SELECT id, phone_e164, now() FROM users
