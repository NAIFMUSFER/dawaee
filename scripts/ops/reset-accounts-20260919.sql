-- One-off production reset requested by the project owner on 2026-09-19.
...
-- Scope is the ten accounts inventoried before the cutoff, never later signups.
...
DO $reset$
DECLARE
  cutoff constant timestamptz := '2026-09-19T13:47:21.389244+00:00';
  fingerprint constant text := 'ca9d122e7f2f17a836be9a70e8459870c226e89964b7a962a0a2cb8c790bed41';
  batch constant text := 'owner-account-reset-20260919';
  account_ids uuid[];
  profile_ids uuid[];
  old_phones text[];
  old_emails text[];
...
BEGIN
  ...
  IF cardinality(account_ids) IS DISTINCT FROM 10 OR
  ...
  IF EXISTS (SELECT 1 FROM public.users WHERE id = ANY(account_ids)
      AND (disabled_at IS NOT NULL OR deletion_requested_at IS NOT NULL)) THEN
  ...
  IF cardinality(profile_ids) IS DISTINCT FROM 10 THEN
  ...
  UPDATE public.users SET disabled_at = now(), deletion_requested_at = now(),
      phone_e164 = NULL, email = 'reset-' || id::text || '@deleted.invalid',
      display_name = 'حساب محذوف', is_admin = false
    WHERE id = ANY(account_ids);
  GET DIAGNOSTICS changed = ROW_COUNT;
  IF changed <> 10 THEN RAISE EXCEPTION 'Expected exactly ten retired accounts'; END IF;
  ...
  INSERT INTO public.audit_logs(actor_user_id, actor_role, action, entity_type, request_id, new_value)
    VALUES(NULL, 'admin', 'accounts.registration_reset', 'account_batch', batch,
      jsonb_build_object('account_count',10,'profile_count',10,'cutoff',cutoff,
