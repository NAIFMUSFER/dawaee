...
(function () {
  ...
  const labels = en ? {
    verify: 'Verify your email', reset: 'Reset your password', password: 'New password (at least 10 characters)',
    ...
    verified: 'Your email is verified. Return to TADAWEE.', failure: 'Unable to complete. Try again or request a new link.',
    ...
    verify: 'تأكيد البريد الإلكتروني', reset: 'إعادة تعيين كلمة المرور', password: 'كلمة المرور الجديدة (10 أحرف على الأقل)',
  ...
  const valid = /^[A-Za-z0-9_-]{43}$/.test(token) && (purpose === 'verify' || purpose === 'reset');
  title.textContent = purpose === 'reset' ? labels.reset : labels.verify;
  ...
  document.getElementById('passwords').hidden = purpose !== 'reset';
  password.required = confirm.required = purpose === 'reset';
  if (!valid) { form.hidden = true; message.textContent = labels.invalid; return; }
  let busy = false;
  form.addEventListener('submit', async function (event) {
    event.preventDefault();
    if (busy || !token) return;
    if (purpose === 'reset' && (password.value.length < 10 || password.value !== confirm.value)) {
    ...
    try {
      const response = await fetch('/v1/auth/email/complete', {
        ...
        body: JSON.stringify(purpose === 'reset' ? { token, purpose, newPassword: password.value } : { token, purpose })
      });
      ...
      token = ''; password.value = confirm.value = ''; form.hidden = true;
      message.textContent = purpose === 'reset' ? labels.saved : labels.verified;
