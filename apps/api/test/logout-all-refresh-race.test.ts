      ...
      `SELECT EXISTS (
         SELECT 1
          ...
          WHERE datname = current_database()
            ...
            AND query LIKE 'UPDATE auth_sessions SET revoked_at = now() WHERE user_id = $1%'
       ) AS waiting`,
    );
...
describe('logout-all versus an in-flight refresh', () => {
  it('does not let a refresh descendant survive the successful logout-all response', async () => {
    ...
    try {
      ...
      // Commit the refresh only after logout-all's revocation UPDATE has taken
      // its statement snapshot. A vulnerable implementation resumes that same
      // snapshot after the predecessor lock clears, never seeing the descendant
