...
describe('verified recipient invitation discovery', () => {
  it('shows a retry instead of silently hiding invitations after a network error', async () => {
    ...
    try {
      ...
      expect(h.text()).toContain('invite.incomingLoadFailed');
      h.find('Button', (p: any) => p.label === 'common.retry').onPress(); await h.flush();
      h.requests[1].resolve({ invitations: [{ id: 'phone-invite', patientName: 'Patient', role: 'caregiver', permissions: ['view_schedule'] }] }); await h.flush();
      expect(h.text()).not.toContain('invite.incomingLoadFailed');
      expect(h.find('Button', (p: any) => p.label === 'invite.accept')).toBeTruthy();
