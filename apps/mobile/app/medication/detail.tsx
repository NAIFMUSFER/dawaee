...
function MedicationDetailProfileScreen({ medicationId }: { medicationId: string | undefined }) {
  ...
  const [loading, setLoading] = useState(Boolean(medicationId && activeProfile));
  const [refreshing, setRefreshing] = useState(false);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<string | null>(medicationId ? null : t('error.not_found'));
