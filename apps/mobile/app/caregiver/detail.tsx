...
import { getCaregiverDetailRouteIntent } from '@/navigation/private-navigation';
import {
 ...
 *
 * Browser paths intentionally do not carry the relationship id. The selected
 * caregiver is recovered only from the short-lived, account/profile-bound
 * private-navigation handoff.
 */
...
export default function CaregiverDetailScreen() {
  ...
  const intent = user && activeProfile
    ...
    : null;
  const relationshipId = intent?.relationshipId;
      ...
      <Screen>
        ...
        <Card>
          ...
          <Row style={{ justifyContent: 'space-between' }} gap={theme.spacing.md}>
            <Button label="−" tone="secondary" fullWidth={false} disabled={!isOwner || priority <= MIN_PRIORITY} accessibilityHint={t('escalation.moveEarlier', { number: formatNumber(priority) })} onPress={() => setPriority((p) => Math.max(MIN_PRIORITY, p - 1))} />
            <Txt variant="h3" weight="bold">{t('family.alertOrderValue', { priority: formatNumber(priority) })}</Txt>
            <Button label="+" tone="secondary" fullWidth={false} disabled={!isOwner || priority >= MAX_PRIORITY} accessibilityHint={t('escalation.moveLater', { number: formatNumber(priority) })} onPress={() => setPriority((p) => Math.min(MAX_PRIORITY, p + 1))} />
          </Row>
          {priority === MIN_PRIORITY ? <Badge label={t('family.primaryCaregiver')} fg={theme.colors.primary700} bg={theme.colors.primary100} /> : null}
        </Card>

        {isOwner ? <Button label={t('common.save')} loading={savingPermissions} disabled={!dirtyPermissions} onPress={() => void savePermissions()} testID="save-permissions" /> : null}

        <SectionTitle>{t('notify.title')}</SectionTitle>
        {RULE_CHANNELS.map((channel) => (
          <ChannelRuleCard key={channel} channel={channel} rule={rules[channel]} editable={isOwner} saving={savingChannel === channel} onChange={(patch) => setRule(channel, patch)} onSave={() => void saveRule(channel, rules[channel])} />
        ))}

        {isOwner ? <Button label={t('family.revokeAccess')} tone="danger" onPress={revoke} /> : null}
      </Screen>
    </SafeAreaView>
  );
}

function PermissionRow({ label, on, disabled, onToggle }: { label: string; on: boolean; disabled: boolean; onToggle: () => void }) {
  const theme = useTheme();
  return (
    <Row style={{ justifyContent: 'space-between', minHeight: theme.touch }} gap={theme.spacing.md}>
      <View style={{ flex: 1 }}><Txt variant="body" color={disabled ? theme.colors.ink500 : theme.colors.ink900}>{label}</Txt></View>
      <Switch value={on} onValueChange={onToggle} disabled={disabled} accessibilityRole="switch" accessibilityLabel={label} trackColor={{ false: theme.colors.ink200, true: theme.colors.primary200 }} thumbColor={on ? theme.colors.primary700 : theme.colors.surface} />
    </Row>
  );
}

function ChannelRuleCard({ channel, rule, editable, saving, onChange, onSave }: { channel: RuleChannel; rule: RuleState; editable: boolean; saving: boolean; onChange: (patch: Partial<RuleState>) => void; onSave: () => void }) {
  const { t, formatNumber } = useI18n();
  const theme = useTheme();
  ...
  return (
    <Card>
      <Row style={{ justifyContent: 'space-between' }} gap={theme.spacing.md}>
        <View style={{ flex: 1 }}><Txt variant="bodyLarge" weight="bold">{t(`channel.${channel}`)}</Txt></View>
        <Switch value={rule.enabled} onValueChange={(enabled) => onChange({ enabled })} disabled={!editable} accessibilityRole="switch" accessibilityLabel={t('notify.channelEnabled')} trackColor={{ false: theme.colors.ink200, true: theme.colors.primary200 }} thumbColor={rule.enabled ? theme.colors.primary700 : theme.colors.surface} />
      </Row>

      <Txt variant="caption" weight="bold" color={theme.colors.ink700}>{t('notify.mode')}</Txt>
      <View style={{ gap: theme.spacing.xs }}>
        {CAREGIVER_NOTIFY_MODES.map((mode) => <Button key={mode} label={`${rule.mode === mode ? '✓ ' : ''}${t(`notify.${mode}`)}`} tone={rule.mode === mode ? 'primary' : 'secondary'} disabled={!editable} onPress={() => onChange({ mode })} />)}
      </View>

      {showsThreshold ? (
        <Row style={{ justifyContent: 'space-between' }} gap={theme.spacing.md}>
          <View style={{ flex: 1 }}><Txt variant="bodySmall">{t('notify.threshold')}</Txt></View>
          <Row gap={theme.spacing.sm}>
            <Button label="−" tone="secondary" fullWidth={false} disabled={!editable || rule.consecutiveMissedThreshold <= 1} accessibilityHint={t('notify.threshold')} onPress={() => onChange({ consecutiveMissedThreshold: Math.max(1, rule.consecutiveMissedThreshold - 1) })} />
            <Txt variant="h3" weight="bold">{formatNumber(rule.consecutiveMissedThreshold)}</Txt>
            <Button label="+" tone="secondary" fullWidth={false} disabled={!editable || rule.consecutiveMissedThreshold >= 10} accessibilityHint={t('notify.threshold')} onPress={() => onChange({ consecutiveMissedThreshold: Math.min(10, rule.consecutiveMissedThreshold + 1) })} />
          </Row>
        </Row>
      ) : null}
